...
export abstract class ProductHetImportService {
  static async preview(file: File): Promise<ImportPreviewResponseDto> {
    ...
    const existingProducts = await db.productHets.findMany({
  ...
  static async execute(data: ImportExecutePayloadDto): Promise<ImportExecuteResultDto> {
    ...
    const createResults = await Promise.all(
      data.willCreate.map(async (item) => {
        try {
          const result = await db.productHets.create({
    ...
    const updateResults = await Promise.all(
      data.willUpdate.map(async (item) => {
        try {
          const result = await db.productHets.update({
            where: { id: item.id },
            data: { name: item.name, price: String(item.price) },
          })
          if (!result)
